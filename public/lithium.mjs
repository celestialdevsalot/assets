...
///         Options        ///
//////////////////////////////
const connection = new BareMuxConnection("/bareworker.js?v=2");
...
///           SW           ///
//////////////////////////////
const stockSW = "./ultraworker.js?v=2";
...
async function updateBareMux() {
	if (transportURL != null && wispURL != null) {
		...
		const BareTransport = mod.default;
		const transport = new BareTransport({ wisp: wispURL });
		await connection.setRemoteTransport(transport, transportURL);
	}
}
...
export async function setWisp(wisp) {
	...
	wispURL = proto + location.host + "/wisp/";
	console.log(`lethal.js: setting wisp to ${wispURL}`);
	await updateBareMux();
}
 ...
 * @returns {Promise<void>}
 */
export async function setPEngine(proxy) {
...
export class Tab {
	...
	constructor() {
		...
		this.frame.setAttribute("allow", "display-capture; picture-in-picture; clipboard-write;");
	...
	handleLoad() {
		...
		if (url === "index.html?type=g") url = "celestial://gẚmes";
		...
		if (url === "index.html?type=ms") url = "celestial://music";
